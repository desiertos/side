...
# arg_prov <- geojson_sf('./geo_data/new/provincias.json')
# 
# #["local", "Cantidad de periodistas", "Relación de periodistas por medio", "Cantidad mayoritaria de periodistas por medio", "El 50% de los medios tiene…", "Plataforma predominante", "Impacto de la publicidad oficial", "Impacto predominante de la publicidad oficial en el ingreso", "Forma de retribución predominante de los periodistas", "Antigüedad predominante de los medios", "Incidencia de la agenda de noticias locales", "Tema más popular", "Tema menos popular", "Descripción del medio prototípico", "xmin", "ymin", "xmax", "ymax", "pob", "cant_medios", "cant_periodistas"]
# 
# names(arg_prov) <- c("deserts_count", "relacion_poblacion_residente/medios", "semiforests_count", 
...
#                      "semideserts_percentage", "poblacion_total", "codpcia", "geometry"
# )
...
datos_locales_geom <- geojson_sf('./geo_data/new/mun_.geojson')

# c("localidad", "pob", "cant_medios", "cant_periodistas", "pobXmedios", "pobXperiodistas", "categoria", "provincia")
# dput(names(datos_locales_geom))

names(datos_locales_geom) <- c("localidad", "pob", "pobXmedios", 
  "cant_medios", "cant_periodistas", "provincia", 
  "pobXperiodistas", "tipo_entidade", 
  "categoria", "nome_local", "codpcia", "geometry")
...
lista_locais <- bind_rows(
  ...
  provincias %>%
    mutate(local = paste0(nam, " (Provincia)"),
           nome = nam,
           tipo = "provincia") %>%
...
jsonlite::write_json(locales_export, "../desiertos.github.io/data/locales_dash.json")
